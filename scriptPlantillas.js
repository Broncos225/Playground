...
window.addEventListener('load', function () {
    ...
    // Además, creamos una bandera en localStorage para verificar intentos previos
    let intentoPrevio = localStorage.getItem('intentoPrevio');

    if (nombreAsesorActual !== nombreAsesorPermitido && intentoPrevio !== 'true') {
        // Guardar que ya hubo un intento previo
        localStorage.setItem('intentoPrevio', 'true');

        // Redirigir al usuario o mostrar mensaje de acceso denegado en el primer intento
    ...
    } else if (nombreAsesorActual !== nombreAsesorPermitido && intentoPrevio === 'true') {
        // Si ya hubo un intento previo, bloquear acceso de nuevo
        alert('Acceso denegado.');
        window.location.href = 'https://broncos225.github.io/Playground/index.html'; // Cambia esto a la URL de la página de redirección
    } else {
        // Si el nombreAsesorActual coincide con el permitido
        localStorage.setItem('intentoPrevio', 'false'); // Resetear la bandera si el acceso es permitido
