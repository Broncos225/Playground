...
async function actualizarUINotificaciones() {
    const checkbox = document.getElementById('activarNotificaciones');
    const textoEstado = document.getElementById('textoEstadoNotificaciones');
    const btnSolicitar = document.getElementById('btnSolicitarNotificaciones');

    if (!checkbox || !textoEstado) return;

    try {
        // Verificar estado actual de OneSignal
        ...
        if (typeof OneSignalDeferred !== 'undefined') {
            OneSignalDeferred.push(async function (OneSignal) {
                ...
                permissionState = await OneSignal.Notifications.permission;
                ...
                if (isSubscribed) {
                    textoEstado.innerHTML = '<i class="fas fa-check-circle" style="color: #28a745;"></i> Notificaciones activadas correctamente';
                    textoEstado.style.background = 'rgba(40, 167, 69, 0.1)';
                    btnSolicitar.style.display = 'none';
                } else if (permissionState === 'denied') {
                    textoEstado.innerHTML = '<i class="fas fa-times-circle" style="color: #dc3545;"></i> Permisos denegados. Actívalos en la configuración del navegador';
                    textoEstado.style.background = 'rgba(220, 53, 69, 0.1)';
                    btnSolicitar.style.display = 'none';
                } else {
                    textoEstado.innerHTML = '<i class="fas fa-info-circle" style="color: #ffc107;"></i> Notificaciones desactivadas';
                    textoEstado.style.background = 'rgba(255, 193, 7, 0.1)';
        ...
        const estadoFirebase = await cargarEstadoNotificaciones();
        if (estadoFirebase) {
            console.log("Estado cargado desde Firebase:", estadoFirebase);
        }

    } catch (error) {
        console.error("Error al actualizar UI:", error);
        textoEstado.innerHTML = '<i class="fas fa-exclamation-triangle" style="color: #dc3545;"></i> Error al verificar estado';
    }
}

// Event listener para el checkbox
document.getElementById('activarNotificaciones')?.addEventListener('change', async function () {
    if (this.checked) {
        // Solicitar permisos
...
document.getElementById('btnSolicitarNotificaciones')?.addEventListener('click', async function () {
    await solicitarPermisosNotificacion();
});
...
window.addEventListener('load', function () {
    ...
    setTimeout(() => {
        ...
        // Listener para cuando cambie el estado de suscripción
        if (typeof OneSignalDeferred !== 'undefined') {
            OneSignalDeferred.push(function (OneSignal) {
                OneSignal.User.PushSubscription.addEventListener('change', async function (event) {
                    console.log("Estado de suscripción cambió:", event);
                    await sincronizarEstadoNotificaciones();
                    await actualizarUINotificaciones();
                });
            });
...
// Sincronizar cada vez que se guarden las configuraciones
document.getElementById('btnGuardarConfiguraciones')?.addEventListener('click', async function () {
    // Tu código existente de guardar...
